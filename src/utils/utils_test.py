    ...
    if max_len != 1: # nothing to map backwards if sequence is too short to be split in the first place
        ...
        for i, sub_prediction in enumerate(sub_predictions):
            if i == 0:
                ...
                # End sequence needs to CLS token + Stride tokens 
                new_sub_prediction = sub_prediction[:,1:,:] # CLS tokens + Stride tokens
            else:
                # Middle sequence needs to CLS token + Stride tokens + SEP token
...
def test_mean(trainer, ds_validation, stride, threshold, path_folds, ignored_labels, id2label, val_id):
